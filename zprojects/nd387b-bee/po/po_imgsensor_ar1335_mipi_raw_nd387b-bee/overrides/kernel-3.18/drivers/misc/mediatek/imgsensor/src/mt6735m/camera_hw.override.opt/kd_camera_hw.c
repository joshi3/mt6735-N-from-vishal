...
#endif
#if defined(GC2165_MIPI_YUV)
	  {SENSOR_DRVNAME_GC2165_MIPI_YUV,
	  {
	   {SensorMCLK, Vol_High, 0},
       {PDN, Vol_Low, 5},
	   {RST, Vol_Low, 5},
    ...
    if (pwInfo.PowerType == AVDD) {
        ...
        if (pinSetIdx == 2) {
            ...
            if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
                ...
                if (TRUE != _hwPowerOn(SUB_DVDD, Vol_1800)) {
                    PK_ERR("[CAMERA SENSOR] Fail to enable digital power\n");
                    return FALSE;
                }
        ...
        if (pwInfo.Voltage == Vol_High) {
            ...
            if (mtkcam_gpio_set(pinId, PDN, pinSet[pinId][IDX_PS_CMPDN + IDX_PS_ON])) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
            ...
            if (mtkcam_gpio_set(pinId, PDN, pinSet[pinId][IDX_PS_CMPDN + IDX_PS_OFF])) {
        ...
        if (pwInfo.Voltage == Vol_High) {
            ...
            if (mtkcam_gpio_set(pinId, RST, pinSet[pinId][IDX_PS_CMRST + IDX_PS_ON])) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
            ...
            if (mtkcam_gpio_set(pinId, RST, pinSet[pinId][IDX_PS_CMRST + IDX_PS_OFF])) {
                PK_ERR("[CAMERA SENSOR] set gpio failed!!\n");
            }
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name)
{
	...
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
					PK_ERR("[CAMERA SENSOR] Fail to disble digital main2 avdd power\n");
					return FALSE;
				}
			} else {
			...
			if (PowerCustList.PowerCustInfo[CUST_SUB_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
					PK_ERR("[CAMERA SENSOR] Fail to disable sub avdd gital power\n");
					return FALSE;
				}
			...
			if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
					PK_ERR("[CAMERA SENSOR] Fail to disable avdd digital power\n");
					return FALSE;
				}
			//} else {
		...
		if (pinSetIdx == 2) {
			if (PowerCustList.PowerCustInfo[CUST_MAIN2_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
				if (TRUE != _hwPowerDown(DVDD)) {
						PK_ERR("[CAMERA SENSOR] Fail to disable dvdd main2 digital power\n");
						return FALSE;
					}
				} else {
				...
				if (PowerCustList.PowerCustInfo[CUST_SUB_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
					if (TRUE != _hwPowerDown(SUB_DVDD)) {
						PK_ERR("[CAMERA SENSOR] Fail to disbble sub dvdd digital power\n");
						return FALSE;
					}
				} else {
				...
				if (PowerCustList.PowerCustInfo[CUST_DVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
					if (TRUE != _hwPowerDown(DVDD)) {
						PK_ERR("[CAMERA SENSOR] Fail to disable dvdd digital power\n");
						return FALSE;
					}
					if (TRUE != _hwPowerDown(SUB_DVDD)) {
						PK_ERR("[CAMERA SENSOR] Fail to disbble sub dvdd digital power\n");
		...
		if (PowerCustList.PowerCustInfo[CUST_DOVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
				PK_ERR("[CAMERA SENSOR] Fail to disable dovdd digital power\n");
				return FALSE;
			}
		} else {
		...
		if (PowerCustList.PowerCustInfo[CUST_AFVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
				PK_ERR("[CAMERA SENSOR] Fail to disable afvdd digital power\n");
				return FALSE;
			}
