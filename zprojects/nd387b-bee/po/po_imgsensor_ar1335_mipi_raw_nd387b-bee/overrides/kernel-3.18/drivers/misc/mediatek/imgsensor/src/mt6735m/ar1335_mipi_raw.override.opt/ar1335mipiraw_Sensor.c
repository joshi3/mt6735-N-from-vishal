...
#include "ar1335mipiraw_Sensor.h"
#include "gc2165yuv.h"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = AR1335_SENSOR_ID,

	.checksum_value =0xffb1ec31,

	.pre = {
		.pclk = 375000000,                //record different mode's pclk   170309:375000000   170313:345000000
		.linelength = 4614,      //0x1206=4614       //record different mode's linelength   4614
		.framelength = 2709,     //0x0A95    //record different mode's framelength     2786
	...
	.cap = {
		.pclk = 375000000,     //170309:375000000      170313:345000000
		.linelength = 5728,
...
static void preview_setting(void)
{
	...
	mdelay(20);
	////vt_clk = 372mhz, dada rate 750mhz
	write_cmos_sensor_2byte(0x0300, 0x0004); 	// VT_PIX_CLK_DIV  0x0004
	write_cmos_sensor_2byte(0x0302, 0x0001); 	// VT_SYS_CLK_DIV
	write_cmos_sensor_2byte(0x0304, 0x0404); 	// PRE_PLL_CLK_DIV
	write_cmos_sensor_2byte(0x0306, 0x7D7D); 	// PLL_MULTIPLIER 0x7878   170309:7D7D  170313:0x7373
	write_cmos_sensor_2byte(0x0308, 0x000A); 	// OP_PIX_CLK_DIV
	write_cmos_sensor_2byte(0x030A, 0x0001); 	// OP_SYS_CLK_DIV
	write_cmos_sensor_2byte(0x0112, 0x0A0A); 	// CCP_DATA_FORMAT
	write_cmos_sensor_2byte(0x3016, 0x0101); 	// ROW_SPEED
	...
	write_cmos_sensor_2byte(0x0400, 0x0001); 	// SCALING_MODE
	write_cmos_sensor_2byte(0x0404, 0x0020); 	// SCALE_M
	write_cmos_sensor_2byte(0x0342, 0x1206); 	// LINE_LENGTH_PCK               //170310: 0x1206
	write_cmos_sensor_2byte(0x0340, 0x0A95); 	// FRAME_LENGTH_LINES //0x0C52   //0803   
	write_cmos_sensor_2byte(0x0202, 0x094c); 	// COARSE_INTEGRATION_TIME
...
static void capture_setting(kal_uint16 currefps)
{
	...
	write_cmos_sensor_2byte(0x3F3C,0x0002);
	    ...
	    mdelay(20);
            ...
            write_cmos_sensor_2byte(0x0304,0x0404); //170313:0x0404
            write_cmos_sensor_2byte(0x0306,0x7D7D); //170309:7D7D   170313:0x7373
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			LOG_INF("read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x\n",read_cmos_sensor(0x0000),read_cmos_sensor(0x0001),read_cmos_sensor(0x0000));
			if (*sensor_id ==imgsensor_info.sensor_id) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				
				return ERROR_NONE;
			}
			LOG_INF("Read sensor id fail, sensor id: 0x%x\n", *sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	sensor_init();
        {
            static int cnt = 0; 
            ++cnt;
            //skip the 1st open during UE power-on
            if (cnt > 0) { 
